...
get '/bet' do
	if session[:player_bank].to_i <= 0
		@error = "Sorry you have no money in your bank. Game over"
		redirect 'game_over'
	end
	erb :bet
end
...
post '/bet' do
	...
	if session[:player_bet].to_i > session[:player_bank].to_i
		@error = "You only have #{session[:player_bank]}. Re-enter a bet amount"
		halt erb(:bet)
	else
		redirect '/game'
	end
